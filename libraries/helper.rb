...
module Bamboo
  ...
  module Helper
    ...
    def bamboo_uri(host, rest_endpoint)
      URI.parse(bamboo_url(host, rest_endpoint))
    end

    def bamboo_url(host, rest_endpoint)
      "https://#{host}/#{REST_BASE}/#{rest_endpoint}"
    end

    def bamboo_base64_creds(user)
      vault = ChefVault.new('passwords')
      user_vault = vault.user(user)
      Base64.encode64("#{user}:#{user_vault.decrypt_password}").strip!
    end

    def bamboo_put(uri, user, json = nil, success_codes = ['200'])
      http = Net::HTTP.new(uri.host, uri.port)
      http.use_ssl = true
      http.verify_mode = OpenSSL::SSL::VERIFY_NONE

      request = Net::HTTP::Put.new(uri.request_uri)
      request['AUTHORIZATION'] = "Basic #{bamboo_base64_creds(user)}"
    ...
    def bamboo_post(uri, user, json = nil, success_codes = ['200'])
      http = Net::HTTP.new(uri.host, uri.port)
      http.use_ssl = true
      http.verify_mode = OpenSSL::SSL::VERIFY_NONE
    ...
    def bamboo_get(uri, user, success_codes = ['200'])
      http = Net::HTTP.new(uri.host, uri.port)
      http.use_ssl = true
      http.verify_mode = OpenSSL::SSL::VERIFY_NONE
    ...
    def bamboo_delete(uri, user, success_codes = ['200'])
      http = Net::HTTP.new(uri.host, uri.port)
      http.use_ssl = true
      http.verify_mode = OpenSSL::SSL::VERIFY_NONE
    ...
    def check_for_errors(http_response, success_codes)
      ...
      Chef::Application.fatal!('error making bamboo request')
    end
